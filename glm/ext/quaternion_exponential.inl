...
namespace glm
{
	...
	GLM_FUNC_QUALIFIER qua<T, Q> exp(qua<T, Q> const& q)
	{
		...
		if (Angle < epsilon<T>()) return qua<T, Q>();
	...
	GLM_FUNC_QUALIFIER qua<T, Q> log(qua<T, Q> const& q)
	{
		...
		{
			if (q.w > static_cast<T>(0)) return qua<T, Q>(log(q.w),
														  static_cast<T>(0),
														  static_cast<T>(0),
														  static_cast<T>(0));
			if (q.template w<static_cast<T>(0))
				return qua<T, Q>(log(-q.w), pi<T>(), static_cast<T>(0), static_cast<T>(0));else return qua<T, Q>(
	...
	GLM_FUNC_QUALIFIER qua<T, Q> pow(qua<T, Q> const& x, T y)
	{
		...
		if (y > -epsilon<T>() && y < epsilon<T>()) return qua<T, Q>(1, 0, 0, 0);
	...
	template <typename T, qualifier Q>
	GLM_FUNC_QUALIFIER qua<T, Q> sqrt(qua<T, Q> const& x) { return pow(x, static_cast<T>(0.5)); }
}//namespace glm
