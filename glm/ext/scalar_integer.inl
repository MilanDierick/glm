...
namespace glm
{
	namespace detail
	{
		...
		{
			GLM_FUNC_QUALIFIER static vec<L, T, Q> call(vec<L, T, Q> const& v, T) { return v; }
		};

		template <length_t L, typename T, qualifier Q>
		struct compute_ceilShift<L, T, Q, true>
		{
			GLM_FUNC_QUALIFIER static vec<L, T, Q> call(vec<L, T, Q> const& v, T Shift) { return v | (v >> Shift); }
		};
		...
		struct compute_ceilMultiple<true, true>
		{
			...
			{
				if (Source > genType(0)) return Source + (Multiple - std::fmod(Source, Multiple));
		...
		struct compute_floorMultiple<true, true>
		{
			...
			GLM_FUNC_QUALIFIER static genType call(genType Source, genType Multiple)
			{
				if (Source >= genType(0)) return Source - std::fmod(Source, Multiple);
		...
		struct compute_floorMultiple<false, false>
		{
			...
			GLM_FUNC_QUALIFIER static genType call(genType Source, genType Multiple)
			{
				if (Source >= genType(0)) return Source - Source % Multiple;
				genType Tmp = Source + genType(1);
	...
	GLM_FUNC_QUALIFIER genIUType prevPowerOfTwo(genIUType value)
	{
		...
		return isPowerOfTwo(value)
				   ? value
				   : static_cast<genIUType>(static_cast<genIUType>(1) << static_cast<genIUType>(findMSB(value)));
	}
	...
	GLM_FUNC_QUALIFIER genIUType nextMultiple(genIUType Source, genIUType Multiple)
	{
		...
		return detail::compute_ceilMultiple<std::numeric_limits<genIUType>::is_iec559, std::numeric_limits<genIUType
											>::is_signed>::call(Source, Multiple);
	}
	...
	GLM_FUNC_QUALIFIER genIUType prevMultiple(genIUType Source, genIUType Multiple)
	{
		...
		return detail::compute_floorMultiple<std::numeric_limits<genIUType>::is_iec559, std::numeric_limits<genIUType
											 >::is_signed>::call(Source, Multiple);
	}
	...
	GLM_FUNC_QUALIFIER int findNSB(genIUType x, int significantBitCount)
	{
		...
		if (bitCount(x) < significantBitCount) return -1;
