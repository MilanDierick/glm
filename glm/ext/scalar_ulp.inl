	...
	namespace detail
	{
		GLM_FUNC_QUALIFIER float nextafterf(float x, float y)
		{
			...
			if ((ix > 0x7f800000) ||	// x is nan
				...
				return x + y;
			if (abs(y - x) <= epsilon<float>()) return y;		// x=y, return y
			if (ix == 0)
			{				// x == 0
				GLM_SET_FLOAT_WORD(x, (hy & 0x80000000) | 1);// return +-minsubnormal
				t = x * x;
				if (abs(t - x) <= epsilon<float>()) return t;
				return x;
		...
		GLM_FUNC_QUALIFIER double nextafter(double x, double y)
		{
			...
			if (((ix >= 0x7ff00000) && ((ix - 0x7ff00000) | lx) != 0) ||	// x is nan
				...
				return x + y;
			if (abs(y - x) <= epsilon<double>()) return y;									// x=y, return y
			if ((ix | lx) == 0)
			{													// x == 0
				GLM_INSERT_WORDS(x, hy & 0x80000000, 1);		// return +-minsubnormal
				t = x * x;
				if (abs(t - x) <= epsilon<double>()) return t;
				return x;
				// raise underflow flag
			}
			if (hx >= 0)
...
namespace glm
{
	...
	GLM_FUNC_QUALIFIER T nextFloat(T x, int ULPs)
	{
		...
		T temp = x;
		for (int i = 0; i < ULPs; ++i) temp = nextFloat(temp);
		return temp;
	}
	...
	GLM_FUNC_QUALIFIER T prevFloat(T x, int ULPs)
	{
		...
		T temp = x;
		for (int i = 0; i < ULPs; ++i) temp = prevFloat(temp);
		return temp;
	}
