...
namespace glm
{
	...
	GLM_FUNC_QUALIFIER genIUType mask(genIUType Bits)
	{
		...
		return Bits >= sizeof(genIUType) * 8
				   ...
				   : (static_cast<genIUType>(1) << Bits) - static_cast<genIUType>(1);
	}
	...
	GLM_FUNC_QUALIFIER u8vec2 bitfieldDeinterleave(uint16 x)
	{
		uint16 REG1(x);
		uint16 REG2(x >>= 1);
	...
	GLM_FUNC_QUALIFIER uint32 bitfieldInterleave(u16vec2 const& v)
	{
		return detail::bitfieldInterleave<uint16, uint32>(v.x, v.y);
	}

	GLM_FUNC_QUALIFIER u16vec2 bitfieldDeinterleave(uint32 x)
	{
		uint32 REG1(x);
		uint32 REG2(x >>= 1);

		REG1 = REG1 & static_cast<uint32>(0x55555555);
		REG2 = REG2 & static_cast<uint32>(0x55555555);

		REG1 = ((REG1 >> 1) | REG1) & static_cast<uint32>(0x33333333);
		REG2 = ((REG2 >> 1) | REG2) & static_cast<uint32>(0x33333333);

		REG1 = ((REG1 >> 2) | REG1) & static_cast<uint32>(0x0F0F0F0F);
		REG2 = ((REG2 >> 2) | REG2) & static_cast<uint32>(0x0F0F0F0F);

		REG1 = ((REG1 >> 4) | REG1) & static_cast<uint32>(0x00FF00FF);
		REG2 = ((REG2 >> 4) | REG2) & static_cast<uint32>(0x00FF00FF);

		REG1 = ((REG1 >> 8) | REG1) & static_cast<uint32>(0x0000FFFF);
		REG2 = ((REG2 >> 8) | REG2) & static_cast<uint32>(0x0000FFFF);

		return u16vec2(REG1, REG2);
	}
	...
	GLM_FUNC_QUALIFIER uint64 bitfieldInterleave(u32vec2 const& v)
	{
		return detail::bitfieldInterleave<uint32, uint64>(v.x, v.y);
	}

	GLM_FUNC_QUALIFIER u32vec2 bitfieldDeinterleave(uint64 x)
	{
		uint64 REG1(x);
		uint64 REG2(x >>= 1);

		REG1 = REG1 & static_cast<uint64>(0x5555555555555555ull);
		REG2 = REG2 & static_cast<uint64>(0x5555555555555555ull);

		REG1 = ((REG1 >> 1) | REG1) & static_cast<uint64>(0x3333333333333333ull);
		REG2 = ((REG2 >> 1) | REG2) & static_cast<uint64>(0x3333333333333333ull);

		REG1 = ((REG1 >> 2) | REG1) & static_cast<uint64>(0x0F0F0F0F0F0F0F0Full);
		REG2 = ((REG2 >> 2) | REG2) & static_cast<uint64>(0x0F0F0F0F0F0F0F0Full);

		REG1 = ((REG1 >> 4) | REG1) & static_cast<uint64>(0x00FF00FF00FF00FFull);
		REG2 = ((REG2 >> 4) | REG2) & static_cast<uint64>(0x00FF00FF00FF00FFull);

		REG1 = ((REG1 >> 8) | REG1) & static_cast<uint64>(0x0000FFFF0000FFFFull);
		REG2 = ((REG2 >> 8) | REG2) & static_cast<uint64>(0x0000FFFF0000FFFFull);

		REG1 = ((REG1 >> 16) | REG1) & static_cast<uint64>(0x00000000FFFFFFFFull);
		REG2 = ((REG2 >> 16) | REG2) & static_cast<uint64>(0x00000000FFFFFFFFull);

		return u32vec2(REG1, REG2);
	}
