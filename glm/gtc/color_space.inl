...
namespace glm
{
	namespace detail
	{
		...
		struct compute_rgbToSrgb<4, T, Q>
		{
			...
			{
				return vec<4, T, Q>(compute_rgbToSrgb<3, T, Q>::call(vec<3, T, Q>(ColorRGB), GammaCorrection),
									ColorRGB.w);
			}
		};
		...
		struct compute_srgbToRgb
		{
			...
			{
				return mix(
					pow((ColorSRGB + static_cast<T>(0.055)) * static_cast<T>(0.94786729857819905213270142180095),
						vec<L, T, Q>(Gamma)),
