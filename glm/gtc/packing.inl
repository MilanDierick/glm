...
namespace glm
{
	namespace detail
	{
		...
		GLM_FUNC_QUALIFIER uint half2float(uint h)
		{
			return ((h & 0x8000) << 16) | (((h & 0x7c00) + 0x1C000) << 13) | ((h & 0x03FF) << 13);
		}

		GLM_FUNC_QUALIFIER uint floatTo11bit(float x)
		{
			if (x == 0.0f) return 0u;
			if (isnan(x)) return ~0u;
			if (isinf(x)) return 0x1Fu << 6u;
		...
		GLM_FUNC_QUALIFIER float packed11bitToFloat(uint x)
		{
			if (x == 0) return 0.0f;
			if (x == ((1 << 11) - 1)) return ~0;//NaN
			if (x == (0x1f << 6)) return ~0;
			//Inf
		...
		GLM_FUNC_QUALIFIER uint floatTo10bit(float x)
		{
			if (x == 0.0f) return 0u;
			if (isnan(x)) return ~0u;
			if (isinf(x)) return 0x1Fu << 5u;
		...
		GLM_FUNC_QUALIFIER float packed10bitToFloat(uint x)
		{
			if (x == 0) return 0.0f;
			if (x == ((1 << 10) - 1)) return ~0;//NaN
			if (x == (0x1f << 5)) return ~0;
			//Inf
		...
		struct compute_half<4, Q>
		{
			...
			{
				vec<4, int16, Q> const Unpack(detail::toFloat16(v.x),
			...
			GLM_FUNC_QUALIFIER static vec<4, float, Q> unpack(vec<4, uint16, Q> const& v)
			{
				i16vec4 Unpack;
				memcpy(&Unpack, &v, sizeof(Unpack));
				return vec<4, float, Q>(detail::toFloat32(v.x),
	...
	GLM_FUNC_QUALIFIER uint8 packUnorm1x8(float v) { return static_cast<uint8>(round(clamp(v, 0.0f, 1.0f) * 255.0f)); }
	...
	GLM_FUNC_QUALIFIER vec2 unpackUnorm2x8(uint16 p)
	{
		...
		return vec2(Unpack) * static_cast<float>(0.0039215686274509803921568627451); // 1 / 255
	}
	...
	GLM_FUNC_QUALIFIER float unpackSnorm1x8(uint8 p)
	{
		int8 Unpack = 0;
		memcpy(&Unpack, &p, sizeof(Unpack));
		return clamp(static_cast<float>(Unpack) * 0.00787401574803149606299212598425f, // 1.0f / 127.0f
					 -1.0f,
					 1.0f);
	}
	...
	GLM_FUNC_QUALIFIER vec2 unpackSnorm2x8(uint16 p)
	{
		i8vec2 Unpack;
		memcpy(&Unpack, &p, sizeof(Unpack));
		return clamp(vec2(Unpack) * 0.00787401574803149606299212598425f, // 1.0f / 127.0f
					 -1.0f,
					 1.0f);
	}
	...
	GLM_FUNC_QUALIFIER float unpackSnorm1x16(uint16 p)
	{
		int16 Unpack = 0;
		memcpy(&Unpack, &p, sizeof(Unpack));
		return clamp(static_cast<float>(Unpack) * 3.0518509475997192297128208258309e-5f, //1.0f / 32767.0f,
					 -1.0f,
					 1.0f);
	}
	...
	GLM_FUNC_QUALIFIER vec4 unpackSnorm4x16(uint64 p)
	{
		i16vec4 Unpack;
		memcpy(&Unpack, &p, sizeof(Unpack));
		return clamp(vec4(Unpack) * 3.0518509475997192297128208258309e-5f, //1.0f / 32767.0f,
					 -1.0f,
					 1.0f);
	}
	...
	{
		i16vec4 const Unpack(detail::toFloat16(v.x),
							 detail::toFloat16(v.y),
							 detail::toFloat16(v.z),
							 detail::toFloat16(v.w));
		uint64 Packed = 0;
	...
	GLM_FUNC_QUALIFIER vec4 unpackHalf4x16(uint64 v)
	{
		i16vec4 Unpack;
		memcpy(&Unpack, &v, sizeof(Unpack));
		return vec4(detail::toFloat32(Unpack.x),
	...
	{
		return ((detail::floatTo11bit(v.x) & ((1 << 11) - 1)) << 0) | ((detail::floatTo11bit(v.y) & ((1 << 11) - 1)) <<
			11) | ((detail::floatTo10bit(v.z) & ((1 << 10) - 1)) << 22);
	}
	...
	GLM_FUNC_QUALIFIER uint32 packF3x9_E1x5(vec3 const& v)
	{
		...
		Unpack.data.w = static_cast<uint>(ExpShared);
		return Unpack.pack;
	}
	...
	GLM_FUNC_QUALIFIER vec<L, uintType, Q> packUnorm(vec<L, floatType, Q> const& v)
	{
		...
		return vec<L, uintType, Q>(round(
			clamp(v, static_cast<floatType>(0), static_cast<floatType>(1)) * static_cast<floatType>(std::numeric_limits<
				uintType>::max())));
	}
	...
	GLM_FUNC_QUALIFIER vec<L, floatType, Q> unpackUnorm(vec<L, uintType, Q> const& v)
	{
		...
		return vec<L, floatType, Q>(v) * (static_cast<floatType>(1) / static_cast<floatType>(std::numeric_limits<
			uintType>::max()));
	}
	...
	GLM_FUNC_QUALIFIER vec<L, intType, Q> packSnorm(vec<L, floatType, Q> const& v)
	{
		...
		return vec<L, intType, Q>(round(
			clamp(v, static_cast<floatType>(-1), static_cast<floatType>(1)) * static_cast<floatType>(std::numeric_limits
				<intType>::max())));
	}
	...
	GLM_FUNC_QUALIFIER vec<L, floatType, Q> unpackSnorm(vec<L, intType, Q> const& v)
	{
		...
		return clamp(
			vec<L, floatType, Q>(v) * (static_cast<floatType>(1) / static_cast<floatType>(std::numeric_limits<intType
			>::max())),
