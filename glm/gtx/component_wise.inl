...
namespace glm
{
	namespace detail
	{
		...
		struct compute_compNormalize<L, T, floatType, Q, true, true>
		{
			GLM_FUNC_QUALIFIER static vec<L, floatType, Q> call(vec<L, T, Q> const& v)
			{
				...
				return (vec<L, floatType, Q>(v) - Min) / (Max - Min) * static_cast<floatType>(2) - static_cast<floatType
				>(1);
			}
		};
		...
		{
			GLM_FUNC_QUALIFIER static vec<L, floatType, Q> call(vec<L, T, Q> const& v) { return v; }
		};
		...
		{
			GLM_FUNC_QUALIFIER static vec<L, T, Q> call(vec<L, floatType, Q> const& v) { return v; }
		};
	}//namespace detail
	...
	GLM_FUNC_QUALIFIER vec<L, floatType, Q> compNormalize(vec<L, T, Q> const& v)
	{
		...
		return detail::compute_compNormalize<L, T, floatType, Q, std::numeric_limits<T>::is_integer, std::numeric_limits
											 <T>::is_signed>::call(v);
	}
	...
	GLM_FUNC_QUALIFIER vec<L, T, Q> compScale(vec<L, floatType, Q> const& v)
	{
		...
		return detail::compute_compScale<L, T, floatType, Q, std::numeric_limits<T>::is_integer, std::numeric_limits<T
										 >::is_signed>::call(v);
	}
	...
	{
		T Result(0);
		for (length_t i = 0, n = v.length(); i < n; ++i) Result += v[i];
		return Result;
	}
	...
	{
		T Result(v[0]);
		for (length_t i = 1, n = v.length(); i < n; ++i) Result = min(Result, v[i]);
		return Result;
	}
	...
	{
		T Result(v[0]);
		for (length_t i = 1, n = v.length(); i < n; ++i) Result = max(Result, v[i]);
		return Result;
	}
}//namespace glm
