...
namespace glm
{
	...
	GLM_FUNC_QUALIFIER typename tdualquat<T, Q>::part_type& tdualquat<T, Q>::operator[](length_type i)
	{
	...
	template <typename T, qualifier Q>
	GLM_FUNC_QUALIFIER typename tdualquat<T, Q>::part_type const& tdualquat<T, Q>::operator[](length_type i) const
	{
		assert(i >= 0 && i < this->length());
		return (&real)[i];
	}
	...
	template <typename T, qualifier Q>
	GLM_FUNC_QUALIFIER GLM_CONSTEXPR tdualquat<T, Q>::tdualquat(mat<2, 4, T, Q> const& m) { *this = dualquat_cast(m); }

	template <typename T, qualifier Q>
	GLM_FUNC_QUALIFIER GLM_CONSTEXPR tdualquat<T, Q>::tdualquat(mat<3, 4, T, Q> const& m) { *this = dualquat_cast(m); }
	...
	template <typename T, qualifier Q>
	GLM_FUNC_QUALIFIER tdualquat<T, Q> operator+(tdualquat<T, Q> const& q) { return q; }

	template <typename T, qualifier Q>
	GLM_FUNC_QUALIFIER tdualquat<T, Q> operator-(tdualquat<T, Q> const& q) { return tdualquat<T, Q>(-q.real, -q.dual); }
	...
	GLM_FUNC_QUALIFIER vec<3, T, Q> operator*(tdualquat<T, Q> const& q, vec<3, T, Q> const& v)
	{
		...
		return (cross(real_v3, cross(real_v3, v) + v * q.real.w + dual_v3) + dual_v3 * q.real.w - real_v3 * q.dual.w) *
			T(2) + v;
	}
	...
	template <typename T, qualifier Q>
	GLM_FUNC_QUALIFIER tdualquat<T, Q> operator*(T const& s, tdualquat<T, Q> const& q) { return q * s; }
	...
	{
		return tdualquat<T, Q>(qua<T, Q>(static_cast<T>(1), static_cast<T>(0), static_cast<T>(0), static_cast<T>(0)),
							   qua<T, Q>(static_cast<T>(0), static_cast<T>(0), static_cast<T>(0), static_cast<T>(0)));
	}

	template <typename T, qualifier Q>
	GLM_FUNC_QUALIFIER tdualquat<T, Q> normalize(tdualquat<T, Q> const& q) { return q / length(q.real); }
	...
	{
		const qua<T, Q> real = conjugate(q.real);
		const qua<T, Q> dual = conjugate(q.dual);
	...
	GLM_FUNC_QUALIFIER mat<3, 4, T, Q> mat3x4_cast(tdualquat<T, Q> const& x)
	{
		...
		vec<4, T, Q> const a(rr.w + rr.x - rr.y - rr.z,
							 xy - wz,
							 xz + wy,
							 -(x.dual.w * r.x - x.dual.x * r.w + x.dual.y * r.z - x.dual.z * r.y));

		vec<4, T, Q> const b(xy + wz,
							 rr.w + rr.y - rr.x - rr.z,
							 yz - wx,
							 -(x.dual.w * r.y - x.dual.x * r.z - x.dual.y * r.w + x.dual.z * r.x));

		vec<4, T, Q> const c(xz - wy,
							 yz + wx,
							 rr.w + rr.z - rr.x - rr.y,
	...
	{
		return tdualquat<T, Q>(qua<T, Q>(x[0].w, x[0].x, x[0].y, x[0].z), qua<T, Q>(x[1].w, x[1].x, x[1].y, x[1].z));
	}
