...
namespace glm
{
	...
	GLM_FUNC_QUALIFIER bool intersectRayPlane(genType const& orig,
											  genType const& dir,
	...
	GLM_FUNC_QUALIFIER bool intersectRayTriangle(vec<3, T, Q> const& orig,
												 vec<3, T, Q> const& dir,
												 vec<3, T, Q> const& vert0,
												 vec<3, T, Q> const& vert1,
												 vec<3, T, Q> const& vert2,
												 ...
												 T& distance)
	{
		...
		if (det > std::numeric_limits<T>::epsilon())
		{
			...
			if (baryPosition.template x<static_cast<T>(0) || baryPosition.x> det
			)
			return false;
			...
			if ((baryPosition.template y<static_cast<T>(0)) || ((baryPosition.x + baryPosition.y) > det))
				return false;
		}
		if (det < -std::numeric_limits<T>::epsilon())
		{
			...
			// calculate U parameter and test bounds
			baryPosition.x = glm::dot(dist, p);
			if ((baryPosition.x > static_cast<T>(0)) || (baryPosition.x < det)) return false;
			...
			// calculate V parameter and test bounds
			baryPosition.y = glm::dot(dir, Perpendicular);
			if ((baryPosition.y > static_cast<T>(0)) || (baryPosition.x + baryPosition.y < det)) return false;
		}
		else return false;
												  ...
												  genType& position)
	{
		...
		if (position.template y<typename genType::value_type(0) || position.y> typename genType::value_type(1)
		)
		return false;
		...
		if (position.template z<typename genType::value_type(0) || position.y + position.z> typename
			genType::value_type(1)
		)
		return false;
	...
	template <typename genType>
	GLM_FUNC_QUALIFIER bool intersectRaySphere(genType const& rayStarting,
											   ...
											   typename genType::value_type& intersectionDistance)
	{
		...
		if (dSquared > sphereRadiusSquered) return false;
	...
	template <typename genType>
	GLM_FUNC_QUALIFIER bool intersectLineSphere(genType const& point0,
