...
namespace glm
{
	namespace io
	{
		...
		{
			if (!std::has_facet<FTy>(ios.getloc())) ios.imbue(std::locale(ios.getloc(), new FTy));
	...
	namespace detail
	{
		...
		GLM_FUNC_QUALIFIER std::basic_ostream<CTy, CTr>& print_vector_on(std::basic_ostream<CTy, CTr>& os, V const& a)
		{
			...
			if (cerberus)
			{
				...
				if (fmt.formatted)
				{
					...
					os << std::fixed << std::right << std::setprecision(fmt.precision) << std::setfill(fmt.space) << fmt
						.delim_left;
	...
	namespace detail
	{
		template <typename CTy, typename CTr, template<length_t, length_t, typename, qualifier> class M, length_t C,
				  length_t R, typename T, qualifier Q>
	...
	template <typename CTy, typename CTr, typename T, qualifier Q>
	GLM_FUNC_QUALIFIER std::basic_ostream<CTy, CTr>& operator<<(std::basic_ostream<CTy, CTr>& os,
																std::pair<mat<4, 4, T, Q> const, mat<4, 4, T, Q> const>
																const& a)
	{
