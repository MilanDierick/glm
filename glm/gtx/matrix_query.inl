...
namespace glm
{
	...
	{
		bool result = true;
		for (length_t i = 0; result && i < m.length(); ++i) result = isNull(m[i], epsilon);
		return result;
	}
	...
	GLM_FUNC_QUALIFIER bool isIdentity(mat<C, R, T, Q> const& m, T const& epsilon)
	{
		...
		{
			for (length_t j = 0; result && j < glm::min(i, m[0].length()); ++j) result = abs(m[i][j]) <= epsilon;
			if (result && i < m[0].length()) result = abs(m[i][i] - 1) <= epsilon;
			for (length_t j = i + 1; result && j < m[0].length(); ++j) result = abs(m[i][j]) <= epsilon;
		}
		return result;
	}
	...
	{
		bool result(true);
		for (length_t i = 0; result && i < m.length(); ++i) result = isNormalized(m[i], epsilon);
		for (length_t i = 0; result && i < m.length(); ++i)
		{
	...
	GLM_FUNC_QUALIFIER bool isOrthogonal(mat<C, R, T, Q> const& m, T const& epsilon)
	{
		...
		for (length_t i(0); result && i < m.length() - 1; ++i)
			for (length_t j(i + 1); result && j < m.length(); ++j) result = areOrthogonal(m[i], m[j], epsilon);

		if (result)
		{
			mat<C, R, T, Q> tmp = transpose(m);
			for (length_t i(0); result && i < m.length() - 1; ++i)
				for (length_t j(i + 1); result && j < m.length(); ++j) result = areOrthogonal(tmp[i], tmp[j], epsilon);
		}
		return result;
	}
}//namespace glm
