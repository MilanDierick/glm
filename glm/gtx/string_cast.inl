...
namespace glm
{
	namespace detail
	{
		...
		GLM_FUNC_QUALIFIER std::string format(const char* msg, ...)
		{
			...
			if (msg == GLM_NULLPTR) return std::string();
	...
	struct literal<int64_t, false>
	{
		...
		struct compute_to_string<vec<1, bool, Q>>
		{
			...
			{
				return detail::format("bvec1(%s)", x[0] ? LabelTrue : LabelFalse);
			}
		};
		...
		struct compute_to_string<vec<2, bool, Q>>
		{
			...
			{
				return detail::format("bvec2(%s, %s)", x[0] ? LabelTrue : LabelFalse, x[1] ? LabelTrue : LabelFalse);
			}
		};
		...
		struct compute_to_string<vec<1, T, Q>>
		{
			GLM_FUNC_QUALIFIER static std::string call(vec<1, T, Q> const& x)
			{
				...
				std::string FormatStr(format("%svec1(%s)", PrefixStr, LiteralStr));

				return detail::format(FormatStr.c_str(), static_cast<typename cast<T>::value_type>(x[0]));
			}
		};
		...
		struct compute_to_string<vec<2, T, Q>>
		{
			GLM_FUNC_QUALIFIER static std::string call(vec<2, T, Q> const& x)
			{
				char const* PrefixStr  = prefix<T>::value();
				char const* LiteralStr = literal<T, std::numeric_limits<T>::is_iec559>::value();
				std::string FormatStr(format("%svec2(%s, %s)", PrefixStr, LiteralStr, LiteralStr));
		...
		struct compute_to_string<vec<3, T, Q>>
		{
			GLM_FUNC_QUALIFIER static std::string call(vec<3, T, Q> const& x)
			{
				char const* PrefixStr  = prefix<T>::value();
				char const* LiteralStr = literal<T, std::numeric_limits<T>::is_iec559>::value();
				std::string FormatStr(format("%svec3(%s, %s, %s)", PrefixStr, LiteralStr, LiteralStr, LiteralStr));
	...
	GLM_FUNC_QUALIFIER std::string to_string(matType const& x) { return detail::compute_to_string<matType>::call(x); }
}//namespace glm
