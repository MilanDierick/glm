...
namespace glm
{
	namespace detail
	{
		...
		struct compute_isCompNull<2, T, Q>
		{
			...
			{
				return vec<2, bool, Q>((abs(v.x) < epsilon), (abs(v.y) < epsilon));
			}
		};
		...
		struct compute_isCompNull<3, T, Q>
		{
			...
			{
				return vec<3, bool, Q>((abs(v.x) < epsilon), (abs(v.y) < epsilon), (abs(v.z) < epsilon));
			}
		};
		...
		struct compute_isCompNull<4, T, Q>
		{
			...
			{
				return vec<4, bool, Q>((abs(v.x) < epsilon),
									   (abs(v.y) < epsilon),
									   (abs(v.z) < epsilon),
									   (abs(v.w) < epsilon));
			}
	...
	GLM_FUNC_QUALIFIER bool areOrthogonal(vec<L, T, Q> const& v0, vec<L, T, Q> const& v1, T const& epsilon)
	{
		...
		return abs(dot(v0, v1)) <= max(static_cast<T>(1), length(v0)) * max(static_cast<T>(1), length(v1)) * epsilon;
	}
	...
	{
		return vec<4, bool, Q>(abs(v.x) < epsilon, abs(v.y) < epsilon, abs(v.z) < epsilon, abs(v.w) < epsilon);
	}
